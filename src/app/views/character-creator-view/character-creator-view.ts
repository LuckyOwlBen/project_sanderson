...
import {
  ...
  CreationStep,
} from '../../services/character-creation-flow-service';
...
@Component({
  ...
  imports: [MatCard, CommonModule, RouterOutlet, MatButtonModule, MatIconModule],
...
export class CharacterCreatorView implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.flowService.currentStep$.pipe(takeUntil(this.destroy$)).subscribe((step) => {
  ...
  ngOnDestroy(): void {
    ...
    if (this.isLevelUpMode) {
      ...
      if (character && character.pendingLevelPoints > 0) {
        ...
        console.log(
          '[Character Creator] Level-up auto-completed on view exit, pending points:',
  ...
  private async persistCurrentStep(): Promise<void> {
    try {
      ...
      console.warn(
        '[Character Creator] Persist hook failed or not available for current step',
        err
      );
    }
  }
  ...
  private completeLevelUp(): void {
    ...
    if (character && character.pendingLevelPoints > 0) {
      ...
      this.characterState.updateCharacter(character);
      console.log(
        '[Character Creator] Level-up completed, pending points:',
        character.pendingLevelPoints
      );
