...
export class NameApiService {
  ...
  getName(
    characterId: string
  ): Observable<{ name: string; level: number; cultures: string[] } | null> {
    return this.http.get<NameResponse>(`${this.charactersUrl}/${characterId}/name`).pipe(
      map((response) => {
        if (!response?.success) {
          return null;
        }
  ...
  saveName(
    ...
    name: string,
    level: number
  ): Observable<{ name: string; level: number } | null> {
    return this.http
