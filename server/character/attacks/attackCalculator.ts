...
export class AttackCalculator {
  ...
  private getWeaponAttacks(): Attack[] {
    const attacks: Attack[] = [];
    const equippedWeapons = this.character.inventory
      .getAllEquippedItems()
  ...
  private generateWeaponAttack(weapon: InventoryItem): Attack | null {
    ...
    const skillTotal = this.character.skills.calculateSkillTotal(
      skillType,
  ...
  private generateTalentAttack(talent: TalentNode): Attack | null {
    ...
    if (
      talent.actionCost === ActionCostCode.Passive ||
    ...
    console.warn(
      `[DEPRECATED] Talent '${talent.id}' is using text-based attack parsing. Please migrate to attackDefinition.`
    );
    ...
    return {
      ...
      traits: [...(attackDetails.traits || []), ...((talent.otherEffects || []) as string[])],
  ...
  private generateAttackFromDefinition(talent: TalentNode): Attack | null {
    ...
    if (def.weaponType === 'light') {
      attackBonus = this.character.skills.calculateSkillTotal(
      ...
      const lightTotal = this.character.skills.calculateSkillTotal(
        SkillType.LIGHT_WEAPONRY,
        this.character.attributes
      );
      const heavyTotal = this.character.skills.calculateSkillTotal(
        SkillType.HEAVY_WEAPONRY,
        this.character.attributes
      );
  ...
  private parseTalentAttack(talent: TalentNode): {
    ...
    if (desc.includes('weapon attack')) {
      ...
      if (desc.includes('light weapon')) {
        ...
        const athleticsTotal = this.character.skills.calculateSkillTotal(
          SkillType.ATHLETICS,
          this.character.attributes
        );
    ...
    if (attackBonus === 0) {
      ...
      const matched = surgeSkillMap.find(
        (s) => descLower.includes(`${s.key} attack`) || descLower.includes(`using ${s.key}`)
      );
      if (matched) {
        attackBonus = this.character.skills.calculateSkillTotal(
          matched.type,
  ...
  private characterHasExpertiseForItem(itemId: string, itemName: string): boolean {
    ...
    if (!expertise) {
      ...
      return (
        this.character.hasExpertise('Light Weaponry') ||
   ...
   */
  private getRangedBonusesForWeapon(range: string): {
    damageBonus: number;
