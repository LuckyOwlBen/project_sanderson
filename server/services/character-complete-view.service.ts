 ...
 */
export async function getCompleteCharacterView(characterId: string): Promise<CompleteCharacterView> {
  ...
  const cultures = Array.isArray(character.cultures) 
    ? character.cultures.map((c: any) => typeof c === 'string' ? c : (c.culture || c.name || String(c)))
    : [];
  ...
  const skills = {
    ...
    allocated: skillEntries.reduce((sum: number, [_, value]: [string, any]) => sum + (Number(value) || 0), 0)
  };
  ...
  const expertisesArray = Array.isArray(character.selectedExpertises) ? character.selectedExpertises : [];
  const expertises = {
