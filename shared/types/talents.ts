/**
 * Talent System - Shared Type Definitions
 * 
 * Core interfaces for talents, talent trees, and related structures
 * Used by both frontend (UI) and backend (server)
 */

import { BonusEffect } from './bonuses';
import { DefenseType, DamageType } from './attacks';
...
/**
 * Radiant order data associated with character
 */
...
/**
 * UI-specific talent state
 */
export interface TalentsState {
  selectedTreeId?: string;
  selectedPath?: string;
  talentsByTree: { [treeId: string]: string[] }; // talent IDs by tree
