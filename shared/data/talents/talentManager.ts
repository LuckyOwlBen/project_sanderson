...
export class TalentTreeManager {
    ...
    Object.entries(ALL_TALENT_PATHS).forEach(([key, path]) => {
      ...
      path.paths?.forEach((tree) => {
        ...
        for (const [registeredKey, registeredTree] of this.treeByKey.entries()) {
          ...
          if (
            registeredTree.pathName &&
            tree.pathName &&
            registeredTree.pathName.toLowerCase() === tree.pathName.toLowerCase()
          ) {
            treeKey = registeredKey;
            break;
          }
  ...
  getNodesByTier(treeOrPathKey: string, tier: number): TalentNode[] {
    ...
    byPath.talentNodes?.forEach((n) => {
      if (n.tier === tier) nodes.push(n);
    });
    byPath.paths?.forEach((t) =>
      t.nodes?.forEach((n) => {
        if (n.tier === tier) nodes.push(n);
      })
    );
  ...
  findParentPath(treeKey: string): string | null {
    ...
    for (const [pathKey, path] of this.pathByKey.entries()) {
      if (
        path.paths?.some(
          (t) =>
            (t.pathName?.toLowerCase?.() || '').toLowerCase() === k ||
            (t.pathName && t.pathName.toLowerCase() === k)
        )
      ) {
        return pathKey;
      }
   ...
   */
  buildDisplayTreesFromKeywords(
    talentKeywords: Record<string, any>
  ): Array<{ key: string; pathName: string; nodes: Array<any> }> {
    ...
    for (const [talentId, kw] of Object.entries(talentKeywords)) {
      ...
      const rawPrereqs =
        node.prerequisites && node.prerequisites.length > 0
          ? node.prerequisites
          : this.getNodePrerequisites(talentId) || [];
