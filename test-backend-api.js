...
/**
 * Direct Integration Test for Backend API
 * Tests all attack calculation endpoints
 */
...
async function runTests() {
  ...
  await test('Single attack roll', async () => {
    ...
    const expectedTotal = attack.attackRoll.finalRoll + attack.attackRoll.skillModifier + attack.attackRoll.bonusModifiers;
    if (attack.attackRoll.total !== expectedTotal) {
  ...
  // Test 8: Complex damage notation
  ...
  await test('Performance: 10 rapid requests', async () => {
    const promises = [];
    for (let i = 0; i < 10; i++) {
      promises.push(
    ...
    if (res.length !== 10 || !res.every(r => r.success)) {
