...
export class CharacterRepo {
   ...
   * List all characters
   */
  async listCharacters(): Promise<CharacterData[]> {
    if (!db) {
      console.warn('[CharacterRepo] Database not initialized, returning empty list');
      return [];
    }
    try {
      ...
      for (const c of chars) {
        const loaded = await this.loadCharacter(c.id);
        if (loaded) results.push(loaded);
      }
      return results;
    } catch (error) {
      console.error('[CharacterRepo] Error listing characters:', error);
      throw error;
    }
  }
  ...
  async loadCharacter(characterId: string): Promise<CharacterData | null> {
    ...
    try {
      ...
      console.error(`[CharacterRepo] Error loading character ${characterId}:`, error);
      throw error;
    }
  }
   ...
   */
  async createCharacter(): Promise<SaveResult> {
    if (!db) throw new Error('Database not initialized');
    try {
      // Generate character ID based on timestamp
      const characterId = Date.now().toString(36) + Math.random().toString(36).substr(2, 5);

      await db.run('BEGIN TRANSACTION');

      // Insert character with defaults
      await db.run(`
      ...
      // Insert default attributes (all 2s)
      await db.run(`
      ...
      await db.run('COMMIT');
      console.log(`[CharacterRepo] Created character: ${characterId}`);
      return { success: true, id: characterId };
    } catch (error) {
      if (db) await db.run('ROLLBACK');
      console.error(`[CharacterRepo] Error creating character:`, error);
  ...
  async saveCharacter(
    ...
    try {
      ...
      await db.run('COMMIT');
      console.log(`[CharacterRepository] Saved character: ${character.name} (${character.id})`);
      return { success: true, id: character.id };
    } catch (error) {
      if (db) await db.run('ROLLBACK');
      console.error(`[CharacterRepository] Error saving character ${character.id}:`, error);
      return { success: false, error: (error as Error).message };
    }
  }
   ...
   * Delete a character
   */
  async deleteCharacter(characterId: string): Promise<SaveResult> {
    if (!db) throw new Error('Database not initialized');
    try {
      ...
      console.log(`[CharacterRepository] Deleted character: ${characterId}`);
      return { success: true, id: characterId };
    } catch (error) {
